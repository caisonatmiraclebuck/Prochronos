...
class core_auctions  {
	...
	/* =============================================================================
		[TIMELEFT] - SHORTCODE
		========================================================================== */
	function shortcode_timeleft( $atts, $content = null ) { global $wpdb, $userdata, $CORE, $post, $shortcode_tags; $STRING = "";
	...
	add_shortcode( 'BIDDINGBOX', array($this,'shortcode_biddingbox') );
...
return $c;
 ...
 <div class="form-row control-group row-fluid ">
  ...
  <div class="form-row control-group row-fluid ">
	...
	function _init(){ global $userdata;
							...
							case "bid": {
								...
								if( $_POST['bidamount'] > $current_price ){
										...
										if(is_numeric($current_price) && $current_price > 0 && isset($checkme['max_amount']) && ( $checkme['max_amount'] >=  $_POST['bidamount']) ){
											...
											// BID +1
											}elseif( $_POST['bidamount'] + 1 <= $checkme['max_amount'] ){
										...
										// NEW BIDDER IS WINNER
											...
											// BID +1
											}elseif( ($current_price + 1) <= $_POST['bidamount'] ){
       ...
       <span class="label label-default pull-right" onclick="jQuery('.biddingbox').hide();" ><?php echo $CORE->_e(array('account','48')); ?></span>
		...
		<input type="hidden" name="bidtype" value="bid" />
         ...
         <div class="col-md-6">         
			...
			<input type='text' class='form-control input-lg' id='bid_amount' name="bidamount"  placeholder='<?php echo hook_price($current_price+1); ?>' /> 
...
BIDDING BOX
========================================================================== */
function shortcode_biddingbox(){ global $CORE, $post, $userdata, $wpdb;	
...
if($hits == ""){ $hits = 0; }
	...
	if(strlen($aff_p) > 1){
		...
		$bid_btn = "<button class='btn' href='javascript:void(0);' onclick=\"alert('".$CORE->_e(array('auction','54','flag_noedit'))."');\">".$CORE->_e(array('auction','70'))."</button>";	
	}elseif(!$userdata->ID){
		$bid_btn = "<button class='btn' href='javascript:void(0);' onclick=\"alert('".$CORE->_e(array('auction','56','flag_noedit'))."');\">".$CORE->_e(array('auction','70'))."</button>";
	}else{
		$bid_btn = "<button class='btn' href='javascript:void(0);' onclick=\"jQuery('.biddingbox').show();\">".$CORE->_e(array('auction','70'))."</button>";
	}
	
	// GET BUY NOW BUTTON
	$buynow_btn = "";
	if(!$userdata->ID){		
		$buynow_btn = "<button class='btn' href='javascript:void(0);' onclick=\"alert('".$CORE->_e(array('auction','56','flag_noedit'))."');\">".$CORE->_e(array('auction','55'))."</button>";	
	}elseif($userdata->ID == $post->post_author){ 
		$buynow_btn = "<button class='btn' href='javascript:void(0);' onclick=\"alert('".$CORE->_e(array('auction','54','flag_noedit'))."');\">".$CORE->_e(array('auction','55'))."</button>";	
	}else{ 
		$buynow_btn = "<button class='btn' href='javascript:void(0);' onclick=\"jQuery('.buynowbox').show();\">".$CORE->_e(array('auction','55'))."</button>";
	}
    ...
    <table class="table table-bordered" id="auctionbidbox">
 
      <tbody>
      
        <tr class="topbits">
          <td><div><span><?php echo $bid_count; ?></span> <?php echo $CORE->_e(array('auction','88')); ?></div></td>
          <td class="hidden-xs"><div><span><a href="<?php echo get_author_posts_url( $post->post_author ); ?>"><?php echo $user_info->data->display_name; ?></a></span> <?php echo $CORE->_e(array('auction','71')); ?></div></td>
           <td><div><span><?php if($condition == 1){ echo $CORE->_e(array('auction','92')); }else{ echo $CORE->_e(array('auction','93')); } ?></span> <?php echo $CORE->_e(array('auction','91')); ?></div></td>    

          <td class="hidden-xs"><div><span><?php echo number_format($hits); ?></span> <?php echo $CORE->_e(array('single','19')); ?></div></td>
        </tr>
        
       <?php if( ( $expiry_date == "" || strtotime($expiry_date) < strtotime(current_time( 'mysql' )) ) || isset($SHOWPAYMENTFORM) ){ ?>
        
        
        
        <tr>
        <td colspan="5" class="paybits">
        <div class="wrap">
       
        
        <span class="txt">
		
			<?php echo $CORE->_e(array('auction','39')); ?>
        
			<?php if(is_numeric($reserve_price) && $reserve_price != "0" && $price_current < $reserve_price){ ?>
    
                 <br /> <span class="rnm"><?php echo $CORE->_e(array('auction','38')); ?></span>
...
// CHECK IF THEY CAN RELIST
if(is_numeric($reserve_price) && $reserve_price != "0" && $price_current < $reserve_price){ }else{
if(isset($GLOBALS['CORE_THEME']['auction_relist']) && $GLOBALS['CORE_THEME']['auction_relist'] == '1'){ 
		...
		if($checkme['userid'] == $userdata->ID && $price_current > $reserve_price && ( get_post_meta($post->ID,'auction_price_paid',true) == "" || ( isset($SHOWPAYMENTFORM) && $SHOWPAYMENTFORM != false) ) ){ 
				...
				<div id="myPaymentOptions" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
				  ...
				  <div class="modal-body">'.$CORE->PAYMENTS($current_price+$price_shipping, "CART-".$post->ID."-".$userdata->ID."-".date("Ymd"), $post->post_title, $post->ID, $subscription = false).'</div>
				  <div class="modal-footer">
				  '.$CORE->admin_test_checkout().'
				  <button class="btn" data-dismiss="modal" aria-hidden="true">'.$CORE->_e(array('single','14')).'</button></div></div></div></div>
				<!-- End Modal -->';
	...
	if($CORE->FEEDBACKEXISTS($post->ID, $userdata->ID) == false){ 
		...
		$STRING .= "<div class='clearfix'></div><hr><div class='text-center'><a href='".$GLOBALS['CORE_THEME']['links']['myaccount']."/?fdid=".$post->ID."' class='btn btn-lg btn-info'>".$CORE->_e(array('feedback','1'))."</a></div>";
...
?>
        ...
        <tr>      
        <td colspan="5" class="pricebits">
        
        <div>
        
        
        <span class="txt"><?php echo $CORE->_e(array('auction','84')); ?></span>
        
        <span class="pull-right"> <strong><?php echo hook_price($price_current); ?></strong> <?php echo $bid_btn;  ?></span>        
        ...
        <?php if(isset($checkme['username']) && $checkme['username'] != "" && is_numeric($checkme['userid'])){ ?>
        <div>
        <span class="txt"><?php echo $CORE->_e(array('auction','32')); ?></span>
        
        <span class="pull-right">
        
		<i class="fa fa-user"></i> <a href="<?php echo get_author_posts_url( $checkme['userid']); ?>"><?php echo $checkme['username']; ?></a>	 
		</span>
       </div>
       <?php } ?>
        ...
        <?php if($price_bin != "" && is_numeric($price_bin) && $price_bin > 0 ){ ?>  
        <tr>
        <td colspan="5" class="pricebits">
        
            <div>
            
            	<span class="txt"><?php echo $CORE->_e(array('auction','8')); ?>
                ...
                <?php } ?>
           		...
           		<span class="pull-right">
                	<strong><?php echo hook_price($price_bin); ?></strong>                	
					<?php echo $buynow_btn; ?>
                </span>
        ...
        <tr>
        <td colspan="5" class="bidbox buynowbox"> 
